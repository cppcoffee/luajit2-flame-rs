...
static __always_inline ptrdiff_t frame_ftsz(cTValue *f)
{
	...
	uint32_t v = 0;
	bpf_probe_read_user(&v, sizeof(v),
		(const void *)&f->u64 + 4);
...
static __always_inline int frame_type(cTValue *f)    { return frame_ftsz(f) & FRAME_TYPE; }
static __always_inline int frame_typep(cTValue *f)   { return frame_ftsz(f) & FRAME_TYPEP; }
static __always_inline bool frame_islua(cTValue *f)  { return frame_type(f) == FRAME_LUA; }
static __always_inline bool frame_isvarg(cTValue *f) { return frame_typep(f) == FRAME_VARG; }
static __always_inline ptrdiff_t frame_sized(cTValue *f) { return frame_ftsz(f) & ~FRAME_TYPEP; }
...
typedef struct GChead {
	GCHeader;
	uint8_t unused1; uint8_t unused2;
	GCRef env; GCRef gclist; GCRef metatable;
} GChead;
